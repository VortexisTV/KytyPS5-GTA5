...
void TextureCache::InitializeImage(ImageId id) {
	...
	// Use the identity-like neutral LUT when a title binds an empty colour-grading volume. The copy
	// reads a linear layout, the one a tiled image is detiled into, so the LUT is written in that
	// layout directly and nothing is detiled. Any other layout takes the ordinary upload.
	const auto upload_neutral_lut = [&]() {
		const auto   binding  = UploadBinding(image);
		auto         transfer = BuildTextureTransfer(image, binding, TransferDirection::Upload);
		const vk::Extent3D extent {ColorGradingLutSize, ColorGradingLutSize, ColorGradingLutSize};
		if (!transfer.valid || transfer.regions.size() != 1u ||
		    transfer.regions[0].imageExtent != extent) {
			return false;
		}
		auto& region = transfer.regions[0];
		// A zero row length or image height means the rows or slices are tightly packed.
		const size_t row   = std::max<size_t>(region.bufferRowLength, ColorGradingLutSize);
		const size_t slice = row * std::max<size_t>(region.bufferImageHeight, ColorGradingLutSize);
		const auto   lut   = BuildNeutralColorGradingLut();
		std::vector<uint32_t> texels(slice * ColorGradingLutSize);
		for (size_t z = 0; z < ColorGradingLutSize; z++) {
			for (size_t y = 0; y < ColorGradingLutSize; y++) {
				std::copy_n(lut.data() + (z * ColorGradingLutSize + y) * ColorGradingLutSize,
				            ColorGradingLutSize, texels.data() + z * slice + y * row);
			}
		}
		const auto size          = texels.size() * sizeof(uint32_t);
		auto&      source        = m_buffer_cache.GetUtilityBuffer(MemoryUsage::Upload);
		const auto source_offset = source.Copy(texels.data(), size, 16);
		region.bufferOffset      = source_offset;
		image.Upload(transfer.regions, source.Handle(), source_offset, size);
		LOGF("TextureCache: substituted a neutral colour-grading LUT for the zero-filled "
		     "volume at 0x%016" PRIx64 " (binding=%u tile=%u)\n",
		     image.info.data.address, static_cast<uint32_t>(binding),
