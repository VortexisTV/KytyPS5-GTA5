...
void CreatePipelineInternal(GraphicContext& graphics, PipelineCache::Pipeline& pipeline,
	...
	// Pipeline libraries cover the plain vertex + pixel shape only; everything else is monolithic.
	const bool use_library = library_cache != nullptr && !mesh && !tessellation && !rect_list &&
	                         vertex_info.size() == 1 &&
	                         vs_input_info.logical_stage == ShaderType::Vertex;
	...
	if (use_library) {
		library_cache->PrepareLayout(pipeline, vertex_program.id,
		                             ps_active ? pixel_program.id : 0, graphics_stages,
		                             descriptor_bindings);
	} else {
	...
	const bool  library_created =
	    use_library && library_cache->CreatePipeline(
	                       pipeline, vertex_program.id, ps_active ? pixel_program.id : 0, rendering,
	                       static_params, pipeline_info, pre_raster_stage_count, fragment_stage);
	if (!library_created) {
		if (use_library && pipeline.pipeline_layout == nullptr) {
			EXIT("graphics pipeline library left no layout\n");
		}
