...
bool IsInput(IR::TessellationAttribute kind) {
	using Attribute = IR::TessellationAttribute;
	return kind == Attribute::ControlInput || kind == Attribute::EvaluationInput ||
	       kind == Attribute::PatchInput;
}

bool IsPatch(IR::TessellationAttribute kind) {
	using Attribute = IR::TessellationAttribute;
	return kind == Attribute::PatchOutput || kind == Attribute::PatchInput;
}
...
uint32_t TessellationPointer(ValueEmitContext& ctx, const IR::Inst& inst) {
	...
	EXIT_IF(variable == 0);
	const auto storage = IsInput(kind) ? spv::StorageClassInput : spv::StorageClassOutput;
	...
	if (IsPatch(kind)) {
		address = EmitBinaryU32(state, spv::OpISub, address, ConstantU32(state, tess.patch_begin));
	}
	...
	const auto offset = IsPatch(kind) ? address
	...
	if (kind == Attribute::LocalOutput || IsPatch(kind)) {
...
void DefineTessellationInterfaces(EmitterState& state) {
	...
	for (const auto* block: state.program.blocks) {
		for (const auto& inst: *block) {
			...
			used.at(kind)   = true;
			// The HS and TES index one range reflected from the HS, so their arrays match.
			if (IsPatch(static_cast<Attribute>(kind))) {
				EXIT_NOT_IMPLEMENTED(!inst.Arg(1).IsImmediate() ||
				                     inst.Arg(1).U32() < tess.patch_begin ||
				                     inst.Arg(1).U32() + 4u > tess.patch_end);
			}
		}
	}
	...
	for (uint32_t index = 0; index < used.size(); index++) {
		...
		} else if (IsPatch(kind)) {
			type = array(TypeU32Vector(state, 4), (tess.patch_end - tess.patch_begin + 15u) / 16u);
		} else {
			const bool local  = kind == Attribute::LocalOutput || kind == Attribute::ControlInput;
			const auto stride = local ? tess.ls_stride : tess.hs_stride;
		...
		if (kind == Attribute::Factor) {
			...
			state.builder.AddAnnotation(spv::OpDecorate, variable, spv::DecorationLocation,
			                            IsPatch(kind) ? (tess.hs_stride + 15u) / 16u : 0u);
		}
		if (kind == Attribute::Factor || IsPatch(kind)) {
