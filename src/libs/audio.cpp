...
bool Audio::QueueSdlAudio(PortOut* port, const void* data, bool blocking) {
	...
	const auto lead_in_size = AudioInternal::OutputLeadInBytes(
	    queued > 0 ? static_cast<uint32_t>(queued) : 0u, prepared_size, grain_us);
	if (lead_in_size != 0) {
		if (port->started) {
			PerfStats::Add(PerfStats::CounterId::AudioUnderruns);
		}
		const std::vector<uint8_t> lead_in(
		    lead_in_size, static_cast<uint8_t>(SDL_GetSilenceValueForFormat(SdlFormat(port->format))));
		if (!SDL_PutAudioStreamData(port->stream, lead_in.data(), static_cast<int>(lead_in_size))) {
	...
	if (blocking) {
		...
		const auto      target_size = AudioInternal::OutputQueueTargetBytes(prepared_size, grain_us);
		const auto      wait_start  = LibKernel::KernelGetProcessTime();
		while (SDL_GetAudioStreamQueued(port->stream) > static_cast<int>(target_size)) {
...
uint64_t Audio::AudioOutQueuedMicros(Id handle) {
	...
	// The stream counts the bytes put into it, which are in the port's own format.
	const auto frame_bytes = BytesPerSample(port.format) * OutputChannels(port);
	const auto queued      = SDL_GetAudioStreamQueued(port.stream);
	if (frame_bytes == 0 || queued <= 0) {
		return 0;
	}
	const uint64_t frames = static_cast<uint64_t>(queued) / frame_bytes;
	return frames * 1000000u / port.freq;
}
